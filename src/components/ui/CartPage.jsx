  ...
  if (cart.length === 0) {
    return (
      <div className="container mx-auto py-8">
        <h1 className="text-2xl font-bold mb-4">{t("cart.cart")}</h1>
        <p className="text-gray-700">{t("cart.noCartItem")}</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4 sm:p-6">
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Summary Card */}
        <div className="lg:sticky lg:top-6 h-fit bg-white p-6 rounded-xl shadow">
          ...
          <p className="text-green-600 text-sm mt-1">
            <span className="text-gray-600">{t("cart.youSaved")}:</span>{" "}
            {formattedDiscount} ({discountPercentage}%)
          </p>

          <label className="flex items-center text-sm text-gray-600 mt-2 accent-pink-400">
            <input type="checkbox" className="mr-2 cursor-pointer" />
            {t("cart.giftOption")}
          </label>

          <button className="mt-6 bg-pink-400 hover:bg-pink-300 text-black font-semibold py-2 px-4 rounded w-full cursor-pointer duration-150 ease-linear">
            {t("cart.proceedToBuy")}
          </button>

          <div className="mt-4 text-sm text-gray-600 border-t pt-2 text-left">
            <p>
